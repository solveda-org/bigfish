...
public class TxtMessageServices {
    ...
    public static Map<String, Object> sendTxtMessageFromTemplate(DispatchContext dctx, Map context) 
    {
        ...
        {   
        	...
        	{
            	return ServiceUtil.returnError(UtilProperties.getMessage("OSafeAdminUiLabels", "TxtMsgCustomerTxtPreferenceError", UtilMisc.toList(partyId), locale));
        	}
        	if(isPartyTextPreferenceTrue && UtilValidate.isNotEmpty(toCell))
        	{
	            ...
	            {
	            	return ServiceUtil.returnError(UtilProperties.getMessage("OSafeAdminUiLabels", "TxtMsgTemplateNotFoundError", locale));
	            }
	            if (content.getString("statusId").equals(""))
	            {
	            	return ServiceUtil.returnError(UtilProperties.getMessage("OSafeAdminUiLabels", "TxtMsgTemplateStatusError", locale));
	            }
				...
				{
	            	return ServiceUtil.returnError(UtilProperties.getMessage("OSafeAdminUiLabels", "TxtMsgTemplateEmptyError", locale));
				}
